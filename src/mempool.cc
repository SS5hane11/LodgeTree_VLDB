...
//     // long long first_pool_size = 14327352832;
...
//         // if(i == 0)
//         //     tm_pools[i].init(tm_buf+i*first_pool_size, first_pool_size, 4096, strdup(name));
//         // // else if(i == 1)
//         // //     tm_pools[i].init(tm_buf+i*first_pool_size, first_pool_size, 4096, strdup(name));
//         // else
...
void threadNVMPools::init (int num_workers, const char * nvm_file, long long size)
{
    ...
    tn_nvm_file= nvm_file;
    long long last_pool_size = 15032385536;

    // long long size_per_pool= ((size - last_pool_size)/(tm_num_workers-1)/4096)*4096;
    long long size_per_pool= ((size)/(tm_num_workers)/4096)*4096;
    ...
    // sprintf(name, "NVM pool %d", tm_num_workers-1);
    // printf("%s\n", name);
    // tm_pools[tm_num_workers-1].init(tm_buf+size_per_pool*(tm_num_workers-1), last_pool_size, 4096, strdup(name));
